...
const LiveResult = () => {
  ...
  // For Blinking button
const BlinkingButton = ({ timeItem, nextTime, navigationHandler, item, user,idx }) => {
                ...
                {groupedTimes.length === 0 ? (
                  ...
                  groupedTimes.map((pair, idx) => (
                    <View key={idx} style={[styles.timeRow]}>
                      {pair.map(timeItem => (
                        <BlinkingButton timeItem={timeItem} nextTime={nextTime} navigationHandler={navigationHandler} item={item} user={user} idx={idx} />
                      ))}
                    </View>
                  ))
                )}
